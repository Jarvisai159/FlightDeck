...
import { MapPin, TrainFront, Loader2 } from 'lucide-react'
import { searchStationsLocal, type StationOption } from '../../data/stations'
...
export default function StationAutocomplete({ label, value, onChange, placeholder = 'City or station' }: Props) {
  ...
  const displayValue = value && !isOpen ? value.name : query
  ...
  const fetchSuggestions = useCallback(async (term: string) => {
    ...
    const local = searchStationsLocal(term)
    setSuggestions(local)

    // Fetch from API for more results
    try {
      setLoading(true)
      const res = await fetch(`/api/stations?query=${encodeURIComponent(term)}&limit=8`)
      if (res.ok) {
        const data = await res.json()
        if (data.stations && data.stations.length > 0) {
          const apiResults: StationOption[] = data.stations.map((s: any) => ({
            id: s.id,
            name: s.name || '',
            city: s.name?.split(/[,(]/)?.[0]?.trim() || s.name || '',
            country: '',
          }))
          if (apiResults.length > 0) {
            // Merge API + local, dedup by ID
            const seen = new Set<string>()
            const merged: StationOption[] = []
            for (const st of [...apiResults, ...local]) {
              if (!seen.has(st.id)) {
                seen.add(st.id)
                merged.push(st)
              }
      ...
      // API unavailable — local results shown
    } finally {
      setLoading(false)
    }
  }, [])
      ...
      {isOpen && suggestions.length > 0 && (
        <div className="absolute z-50 top-full mt-1 w-full bg-bg-secondary border border-border rounded-md shadow-xl max-h-64 overflow-y-auto">
          {suggestions.map((station, idx) => (
            ...
            >
              <TrainFront className="w-3.5 h-3.5 mt-0.5 shrink-0 text-accent" />
              <div className="min-w-0 flex-1">
                <span className="text-xs font-medium">{station.name}</span>
                {station.country && (
                  <p className="text-[10px] text-text-muted truncate">{station.country}</p>
                )}
              </div>
            </button>
          ))}
